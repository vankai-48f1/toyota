...
abstract class Base
{
    ...
    public function initialize()
    {
        ...
        // \add_filter(WPMETEOR_TEXTDOMAIN . '-frontend-adjust-wpmeteor', array($this, 'frontend_load_settings'), $this->priority, 2);
        $settings = \wpmeteor_get_settings();
        // if (@$settings[$this->id]['enabled'] && (!$this->disabledInUltimateMode || !@$settings['ultimate-reorder']['enabled'])) {
            \add_filter(WPMETEOR_TEXTDOMAIN . '-frontend-adjust-wpmeteor', array($this, 'frontend_adjust_wpmeteor'), $this->priority, 2);
            if (method_exists($this, 'frontend_rewrite')) {
                \add_filter(WPMETEOR_TEXTDOMAIN . '-frontend-rewrite', array($this, 'frontend_rewrite'), $this->priority, 2);
            }
        // }
    }
